...
def ok_click(player_names):
    for idx, name in enumerate(player_names):
        if not name.strip():
            st_javascript(f"alert('Ai cũng có tên!'); document.getElementById('player_name_{idx}').focus();", key=f"alert_missing_name_{idx}")
            return
...
def new_game_screen():
    ...
    for i in range(4):
        ...
        player_name_inputs.append(name)

    col_ok, col_cancel = st.columns([1,1])
    col_ok.button("OK", on_click=ok_click, args=[player_name_inputs])
    ...
    css = r'''
            <style>
                ...
                div[class^='st-key-new_game'] div, div[class*='st-key-new_game'] div{height:0px;}
            </style>
        '''

...
def ok_click(scores, game_section, game_result_list):
    ...
    if sum(scores) != 0:
        ...
        return
    # Append to game_result_list
    game_result_list.append([int(s) for s in scores])
    js_update_results = f"window.localStorage.setItem('game_result_list', '{json.dumps(game_result_list)}');"
    st_javascript(js_update_results, key="game_result_game_result_list_update")
    ...
    st_javascript(js_update_section, key="game_result_game_section_update")

    st.session_state.screen = "game"
    # st.rerun()

def game_result_screen():
    st.title("Add Result")
    # Load game_section from window.localStorage
    js_code = "window.localStorage.getItem('game_section')"
    game_section_json = st_javascript(js_code, key="game_result_game_section")
    if not game_section_json:
        st.title("No game in progress.")
        return
    js_code_results = "window.localStorage.getItem('game_result_list')"
    game_result_list_json = st_javascript(js_code_results, key="game_result_game_result_list")
    game_result_list = []
    ...
    for i, player in enumerate(game_section):
        score = cols[i].number_input(f"{player['name']}", key=f"score_input_{i}", value=0, step=1, format="%d")
        scores.append(score)
    ...
    css = r'''
            <style>
                ...
                div[class^='st-key-game_result'], div[class*='st-key-game_result']{height:0px;}
            </style>
        '''
